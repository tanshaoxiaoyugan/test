...
{
	/* video io bug -> set false */
	__inpaint(false, voWritter);
}
	...
	for (int i = 0; i < m_Raw.rows; i++)
		for (int k = 0; k < m_Raw.cols; k++)
		{
			...
			if (m_Mask.at<unsigned char>(i, k))										/* pixels to be filled */
				m_Inpainted.at<float>(i, k) = MathUtil::geneRandomReal(ExtremeValue[1], ExtremeValue[0]);
		}
...
void CInpainter::__inpaintC2(bool vIsWriteVideo, cv::VideoWriter& voWritter)
{
	...
	for (int i = 0; i < m_Raw.rows; i++)
		for (int k = 0; k < m_Raw.cols; k++)
		{
			...
			if (m_Mask.at<unsigned char>(i, k))										/* pixels to be filled */
			{
				m_Inpainted.at<cv::Vec2f>(i, k)[0] = MathUtil::geneRandomReal(ExtremeValueC1[1], ExtremeValueC1[0]);
				m_Inpainted.at<cv::Vec2f>(i, k)[1] = MathUtil::geneRandomReal(ExtremeValueC2[1], ExtremeValueC2[0]);
			}
		}
	...
	while (m_Pyramid >= 0)															/* different scales */
	{
		...
		log("Pyramid: " + std::to_string(m_Pyramid) + ", Size [" + std::to_string(Size.width) + ", " + std::to_string(Size.height) + "]");
		...
		m_Iters.emplace_back(m_IterCount - LastIterCount);
